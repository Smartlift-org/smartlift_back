class RoutineExercise < ApplicationRecord
  ...
  validates :exercise_id, uniqueness: { scope: :routine_id, message: "is already included in this routine" }
  ...
  def as_json(options = {})
    super(options.merge(
      ...
      exercise: {
        ...
        level_value: exercise.level_value,
  ...
  def validate_exercise_belongs_to_user
    return if exercise.nil? || routine.nil?
    if exercise.user_id.present? && exercise.user_id != routine.user_id
      errors.add(:exercise_id, "does not belong to the same user as the routine")
    end
  end
end 